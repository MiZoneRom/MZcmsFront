					...
					<li v-for="(item,index) in $router.options.routes" v-if="!item.hidden" class="reng-menu">
						...
						<template v-else>
							...
							<ul class="reng-submenu" :class="'submenu-hook-'+index" @mouseover="showMenu(index,true)" @mouseout="showMenu(index,false)">
								<li class="el-menu-item" v-for="child in item.children" :class="$route.path==child.path?'is-active':''" :key="child.path" @click="$router.push(child.path)" v-if="!child.hidden" style="padding: 0 20px;">{{child.name}}</li>
							</ul>
						</template>
					</li>
				</ul>
